    ...
    for node_type in node_types_to_try:
        ...
        if node_type == NodeType.STEM:
            if action == ModificationType.INSERT:
                new_seq, new_struct = modification_engine._insert_stem_pair(sequence, structure, coords)
                action_name = "insert_pair"
            else:
                new_seq, new_struct = modification_engine._delete_stem_pair(sequence, structure, coords)
                action_name = "delete_pair"
        else:
            if action == ModificationType.INSERT:
                new_seq, new_struct = modification_engine._insert_loop_base(sequence, structure, coords)
                action_name = "insert"
            else:
                new_seq, new_struct = modification_engine._delete_loop_base(sequence, structure, coords, min_size)
                action_name = "delete"
...
def generate_test_case(seq_min_len: int, seq_max_len: int, seq_len_mean: float, 
    ...
    for attempt in range(max_attempts):
        try:
            ...
            # Parse new bulge graph
            new_bulge_graph = bulge_parser.parse_structure(mod_result['new_structure'], mod_result['new_sequence'])
            if not new_bulge_graph:
                continue
            ...
            test_case = {
                ...
                'pre_mod_bulgegraph': bulge_graph_to_dict(bulge_graph),
                ...
                'post_mod_bulgegraph': bulge_graph_to_dict(new_bulge_graph),
...
def main():
    ...
    parser.add_argument("--output", type=str, default="bulge_graph_test_cases.json",
